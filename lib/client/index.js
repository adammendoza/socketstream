...
module.exports = function(ss, router) {
  ...
  return {
    ...
    define: function(name, paths, c2,c3,c4,c5) {
      if (clients[name]) {
      ...
      if (typeof paths === "function") {
        client.bundler = paths(ss,options);
        client.bundler.define(client, c2,c3,c4,c5);
      } else {
        client.bundler = defaultBundler;
        client.bundler.define(client, paths);
      }
      return client;
    },

    describeAssets: function(name) {
      var client = clients[name];

      var containerDir = pathlib.join(ss.root, options.dirs.assets);
      var clientDir = pathlib.join(containerDir, client.name);

      var existslib = process.version.split('.')[1] === '6' && path || fs;
